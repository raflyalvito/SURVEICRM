...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (!surveyData) {
      ...
      errorMsgEl.textContent = 'Belum ada survei yang aktif. Silakan buat survei terlebih dahulu di Dashboard Admin.';
    ...
    descEl.textContent  = surveyData.description || 'Silakan lengkapi kuesioner berikut sesuai dengan pengalaman dan penilaian Anda.';
    ...
    console.error(err);
    loadingEl.classList.add('hidden');
    errorMsgEl.textContent = 'Terjadi kesalahan saat memuat survei: ' + err.message;
    errorEl.classList.remove('hidden');
  }
  ...
  window.selectScale = function(qId, num) {
    ...
    document.querySelectorAll(`button[data-scale-btn="${qId}"]`).forEach(btn => {
  ...
  function updateProgress(questions) {
    ...
    const answered = required.filter(q => userAnswers[q.id] && userAnswers[q.id] !== '');
    const pct = Math.round((answered.length / required.length) * 100);
  ...
  formEl.addEventListener('submit', async (e) => {
    ...
    if (!nim) {
      ...
      window.UI.showToast('NIM wajib diisi sebelum mengirim jawaban!', 'warning');
